...
from core.models.Model import CSCL, Adapter
...
from core.utils.asp_loss import AdaptiveSupervisedPatchNCELoss
...
def build_downstream_datasets(args):
    ...
    # BCNB
    bcnb_dataset = SlideDataset( # TODO:这个没用，后面删掉
        dataset_name = "BCNB",
        csv_path= "/mnt/lpai-dione/ssai/cvg/team/qiuyin/zyz/classification/CSCL_MyACROBAT_Wintraloss/dataset_csv/BCNB/BCNB.csv",
        high_features_path="/mnt/lpai-dione/ssai/cvg/team/qiuyin/zyz/classification/results/BCNB2/processing_slides_933_mag_10x_patchsize_256/patch_embeddings",
        low_features_path="/mnt/lpai-dione/ssai/cvg/team/qiuyin/zyz/classification/results/BCNB2/processing_slides_933_mag_10x_patchsize_256/patch_embeddings",
        modalities=["HE"],
        train=False
    )
...
def setup_dataset(args):
    ...
    dataset = SlideDataset(
        ...
        high_features_path=args.high_data_root_dir,
        low_features_path=args.low_data_root_dir,
...
def setup_model(args):
    ...
    #         print(f"- {name}: {param.shape}, 参数数量: {num_params:,}")
...
def setup_losses(args):
        ...
        # ASP loss
        if args.ASP_loss == "asp":
            criterionASP = AdaptiveSupervisedPatchNCELoss(args).to(DEVICE)
        else:
            criterionASP = None

        return loss_fn_interMod, loss_fn_interMod_local, loss_fn_intraMod, criterionASP