...
class Adapter(nn.Module):
    def __init__(self, config, stain_encoding=False):
        ...
        # 低分辨率特征的adapter
        self.adapter_5_low = nn.Sequential(
            nn.Linear(
                self.config.patch_embedding_dim,
                self.config.patch_embedding_dim // 4
            ),
        ...
        # 高分辨率特征的adapter
        self.adapter_10_high = nn.Sequential(
            nn.Linear(
                self.config.patch_embedding_dim,
                self.config.patch_embedding_dim // 4
            ),
    ...
    def get_features(self, feats, device):
        feats = feats.to(device)
        # 分离高分辨率和低分辨率特征
        high_feature = feats[..., :512]
        low_feature = feats[..., 512:]
        
        # 通过各自的adapter处理特征
        low_feature = self.adapter_5_low(low_feature)
        high_feature = self.adapter_10_high(high_feature)
    ...
    def forward(self, data, device, train=True, n_views = 1, custom_stain_idx=None, return_attention=False):
        '''
        data
        dict_keys(['all_wsi_feats', 'modality_labels', 'slide_ids'])
        data['all_wsi_feats'].shape torch.Size([10, 5, 2048, 1024])
        '''
        high_low_feature = data['all_wsi_feats']
        high_feature = high_low_feature[..., :512]  # 取前512维
        low_feature = high_low_feature[..., 512:]
        
        low_feature = self.adapter_5_low(low_feature)
        high_feature = self.adapter_10_high(high_feature) # torch.Size([10, 5, 2048, 512])
        # pdb.set_trace()
        return low_feature, high_feature
...
class CSCL(nn.Module):
    ...
    def forward(self, data, device, train=True, n_views = 1, custom_stain_idx=None, return_attention=False):
        ...
        # 组织数据
        all_wsi_feats = data['all_wsi_feats'].to(device)
                    # mix_feature_per_modal = multi_scale_data_per_modal
        ...
        if train:
            ...
            all_wsi_feats = all_wsi_feats.view(bs*n_mod, n_tokens, d_in) # torch.Size([50, 2048, 342])
            ...
            # slide_embeddings: [10, 3, 512, 4]
            # token_embeddings: [10, 2048, 512, 4]
...
class ABMILEmbedder(nn.Module):
    ...
    def forward(
        ...
        embeddings = self.pre_attn(bags) # bags: [20, 2048, 1056], embedding: [20, 2048, 2048]
        
        if self.n_heads > 1: # self.n_heads: 4
        ...
        token_embeddings = embeddings # [20, 2048, 512, 4]
        ...
        attention = torch.stack(attention, dim=-1) # return post softmax attention, [10, 2048, 1, 4]
        raw_attention = torch.stack(raw_attention, dim=-1) # return post softmax attention, [10, 2048, 1, 4]
