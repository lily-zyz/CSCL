        ...
        if stain_mask.sum().item() > 1:
            ...
            if loss_fn_interMod:
                ...
                if args.global_loss == "info-nce":
                    print('HE_for_stain:',HE_for_stain.shape) # torch.Size([24, 512])
                    print('stain_ind:',stain_ind.shape) # torch.Size([24, 512])
            ...
            if loss_fn_interMod_local:
                ...
                print('HE_tokens:',HE_for_stain.shape) # torch.Size([24, 512])
                print('IHC_tokens:',stain_ind.shape) # torch.Size([24, 512])
            ...
            #     print('HE_for_stain_view1:',HE_for_stain_view1.shape) # torch.Size([24, 512]) （B，dim）
            #     print('HE_for_stain_view2:',HE_for_stain_view2.shape) # torch.Size([24, 512])
...
def train_loop(args, loss_fn_interMod, loss_fn_interMod_local, loss_fn_intraMod, ssl_model, epoch, dataloader, optimizer, scheduler_warmup, scheduler):
    ...
    for b_idx, data in enumerate(dataloader):
        ...
        with torch.amp.autocast(device_type="cuda", dtype=torch_precision):
            ...
            # get model outputs
            # print(f"data: {data}")
            wsi_embs, token_embs = ssl_model(data=data, device=DEVICE, n_views=n_views)
        ...
        all_embeds.extend(wsi_embs['HE'].detach().to(torch.float32).cpu().numpy())
